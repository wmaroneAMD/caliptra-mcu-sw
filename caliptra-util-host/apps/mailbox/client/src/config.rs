...
impl TestConfig {
    ...
    pub fn load_default() -> Result<Self> {
        ...
        loop {
            ...
            let mailbox_config = current_dir.join("apps").join("mailbox").join("test-config.toml");
            if mailbox_config.exists() {
                return Self::from_file(mailbox_config);
            }
            
            // Try caliptra-util-host subdirectory
            let caliptra_config = current_dir.join("caliptra-util-host").join("test-config.toml");
            if caliptra_config.exists() {
                return Self::from_file(caliptra_config);
            }
            
            // Try caliptra-util-host/apps/mailbox subdirectory
            let caliptra_mailbox_config = current_dir.join("caliptra-util-host").join("apps").join("mailbox").join("test-config.toml");
