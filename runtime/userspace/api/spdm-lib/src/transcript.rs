...
impl Transcript {
    ...
    pub async fn append(
        ...
        match context {
            ...
            TranscriptContext::L1 => self.append_l1(self.spdm_version, session_info, data, crypto).await,
    ...
    async fn append_m1(&mut self, data: &[u8], crypto: &mut dyn SpdmCryptoProvider) -> TranscriptResult<()> {
        if let Some(ctx) = &mut self.hash_ctx_m1 {
